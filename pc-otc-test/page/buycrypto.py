# coding:utf-8
buy_crypto_url = "http://wex.test.tigerft.com/market/buy"
...
class BuyCryptoPage(BasePage):
    ...
    # 购买页面标题
    h2_buy_crypto = ("xpath", '//*[@id="app"]/main/div/div/div/div[2]/h2')
    # 广告列表
    tr_adverts = ("tag name", 'tr')
    # 第一条广告
    frist_advert = ("xpath", '//*[@id="app"]/main/div/div/div/div[2]/div/table/tbody/tr/td[5]/button')
    # 第一个广告主名称
    frist_advert_name = ("xpath", '//*[@id="app"]/main/div/div/div/div[2]/div/table/tbody/tr/td[1]/h2/a')
    ...
    @staticmethod
    def side_crypto(data, buy_crypto_elements):
        """选择数字币币和广告方向
        :param data:数据源
        :param buy_crypto_elements:元素集
        """
        if data['Type'] == 'Buy':
            buy_crypto_elements[0].click()
            if data['Crypto'] == 'USDT':
                buy_crypto_elements[2].click()
            elif data['Crypto'] == 'USDC':
                buy_crypto_elements[3].click()
            elif data['Crypto'] == 'TUSD':
                buy_crypto_elements[4].click()
        else:
            buy_crypto_elements[1].click()
            if data['Crypto'] == 'USDT':
                buy_crypto_elements[2].click()
            elif data['Crypto'] == 'USDC':
                buy_crypto_elements[3].click()
    ...
    def set_legal_currency(self, data, buy_crypto_driver):
        """选择法币
        :param data:数据源
        :param buy_crypto_driver:webdriver
        """
        if data['Currency'] == 'HKD':
            buy_crypto_driver.click_ul_li(self.div_legal_currency, self.ul_legal_currency, self.li_hkd, is_move=False)
        elif data['Currency'] == 'TWD':
            buy_crypto_driver.click_ul_li(self.div_legal_currency, self.ul_legal_currency, self.li_twd, is_move=False)
        elif data['Currency'] == 'USD':
            buy_crypto_driver.click_ul_li(self.div_legal_currency, self.ul_legal_currency, self.li_usd, is_move=False)
        elif data['Currency'] == 'IDR':
            buy_crypto_driver.click_ul_li(self.div_legal_currency, self.ul_legal_currency, self.li_idr, is_move=False)
        elif data['Currency'] == 'VND':
            buy_crypto_driver.click_ul_li(self.div_legal_currency, self.ul_legal_currency, self.li_vnd, is_move=False)
        elif data['Currency'] == 'KHR':
            buy_crypto_driver.click_ul_li(self.div_legal_currency, self.ul_legal_currency, self.li_khr, is_move=False)
